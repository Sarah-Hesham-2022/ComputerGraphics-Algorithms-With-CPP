...
LRESULT CALLBACK WindowProcedure (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_LBUTTONUP:
    {
        x2=LOWORD(lParam);
        y2=HIWORD(lParam);
        radius1=(sqrt((x2-x1)*(x2-x1)+(y2-y4)*(y2-y4)));
        break;
    }
    case WM_RBUTTONDOWN:
    {
        ...
        x3=LOWORD(lParam);
        y3=HIWORD(lParam);
        double radius2=(sqrt((x3-x1)*(x3-x1)+(y3-y4)*(y3-y4)));
        DrawEllipse(hdc,x1,y4,int(radius1),int(radius2),RGB(255,0,0));
        ReleaseDC(hwnd,hdc);
        break;
    }
    case WM_DESTROY:
