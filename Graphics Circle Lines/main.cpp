...
void DrawCircleBresenhams(HDC hdc,int xc,int yc,int radius,COLORREF c)
{
    int x=0;
    int y=radius;
...
void DivideCircle(HDC hdc,int xc,int yc,int radius,COLORREF c)
{
    DrawCircleBresenhams(hdc,xc,yc,radius,c) ;
    DrawLineBresenhams(hdc,xc,yc,(xc),(yc+radius),c);
    DrawLineBresenhams(hdc,xc,yc,(xc),(yc-radius),c);
    DrawLineBresenhams(hdc,xc,yc,(xc+radius),(yc),c);
    DrawLineBresenhams(hdc,xc,yc,(xc-radius),(yc),c);
    DrawLineBresenhams(hdc,xc,yc,(xc+radius/sqrt(2)),(yc+radius/sqrt(2)),c);
    DrawLineBresenhams(hdc,xc,yc,(xc-radius/sqrt(2)),(yc+radius/sqrt(2)),c);
    DrawLineBresenhams(hdc,xc,yc,(xc+radius/sqrt(2)),(yc-radius/sqrt(2)),c);
    DrawLineBresenhams(hdc,xc,yc,(xc-radius/sqrt(2)),(yc-radius/sqrt(2)),c);
...
LRESULT CALLBACK WindowProcedure (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_LBUTTONUP:
    {
        ...
        //DrawCircleBresenhams(hdc,xs,ys,r,RGB(255,0,0)) ;
        DivideCircle(hdc,xs,ys,r,RGB(255,0,0));
        ReleaseDC(hwnd,hdc);
        break;
    }
    case WM_DESTROY:
